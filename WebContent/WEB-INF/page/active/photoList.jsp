    ...
    <section class="hbox stretch"> 
      ...
      <section id="content">
        <section class="panel panel-1 panel-default">
           <header class="panel-heading">相册管理</header>
           <div class="row text-sm wrapper">
              <div>
                <a href="#modal_photo" data-toggle="modal" class="btn btn-s-md btn-danger btn-rounded" style="float: right;" id="btn_add_photo">添加图片</a>
              </div>
          </div>
          ...
          <div class="panel-body">
            <form role="form">
               ...
               <div class="form-group">
                  <label>图片</label>
                  <input type="hidden" id="photo_img"/>
              ...
              <div class="form-group">
                  <label>标识</label>
                  <input type="text" class="form-control" placeholder="标识" id="photo_key">
              </div>
               <div class="form-group">
                  <label>描述</label>
                  <input type="text" class="form-control" placeholder="描述" id="photo_text">
              </div>
              <button type="submit" class="btn btn-s-md btn-success" style="display:block;margin: 20px auto 0" id="btn_submit_photo" data-dismiss="modal">添加</button>
            </form> 
          </div>
        </section>
      </div>
...
<script src="/themes/active/js/photo.js"></script>
<script type="text/javascript">
		  $("#file").fileinput({
			  language: 'zh',
			  uploadUrl: 'active/photoController/uploadFile.do',
		      'allowedFileExtensions' : ['jpg', 'png','gif'],
		  });
		  $('#file').on('fileuploaded', function(event, data, previewId, index) {
		      var response = data.response;
		      if(response.success){
		    	  $("#photo_img").val(response.obj);
		      }
		  });
		  </script>
</body>
